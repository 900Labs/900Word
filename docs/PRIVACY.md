...
Sprint 031 adds DOCX import/export conversion while keeping ODT canonical. Opening a `.docx` validates the path and package in Rust, imports supported content into an unsaved dirty document, and does not expose the source path or filename to frontend document state. DOCX export writes only to a user-entered `.docx` path after backend extension and traversal validation. Status exposed to the frontend includes only format and byte length. The converter does not fetch remote relationships, import media payloads, run macros, invoke external converters, add telemetry, read usernames or hostnames, create accounts, or send document content to network services.
